      ...
      <div className="main-content">
        <div className="dashboard-hero">
          ...
          <div className="assistant-panel">
            ...
            <div className="quick-grid">
              ...
              <Link to="/investimentos">
                Como inserir novo investimento?
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="row g-3 mb-4">
          <div className="col-md-4">
            <MetricCard label="Recebimentos" value={totalReceitas} icon="bi-arrow-down-circle" hint="Entrada consolidada" />
          </div>
          <div className="col-md-4">
            <MetricCard label="Gastos" value={totalGastos} icon="bi-arrow-up-circle" hint="Saída operacional" />
          </div>
          <div className="col-md-4">
            <MetricCard label="Investimentos" value={totalInvestimentos} icon="bi-pie-chart" hint={`${percentual}% eficiência`} />
          </div>
        </div>
        <div className="row g-3 mb-4">
          <div className="col-sm-6 col-xl-3">
            <MetricCard label="Saldo total" value={saldo} icon="bi-wallet2" hint="Visão consolidada" />
          </div>
          <div className="col-sm-6 col-xl-3">
            <MetricCard label="Receitas" value={totalReceitas} icon="bi-arrow-down-circle" hint="+12,4% no mês" />
          </div>
          <div className="col-sm-6 col-xl-3">
            <MetricCard label="Despesas" value={totalGastos} icon="bi-arrow-up-circle" hint="Controle operacional" />
          </div>
          <div className="col-sm-6 col-xl-3">
            <MetricCard label="Investimentos" value={totalInvestimentos} icon="bi-pie-chart" hint="Carteira ativa" />
          </div>
        </div>
        <div className="row g-3">
          <div className="col-xl-8">
            <DashboardChart 
              ...
              receitas={[4200, 6100, 5400, 7200, 8600, 9300]} 
              despesas={[2300, 2800, 2600, 3100, 3700, 4200]} 
            />
          </div>
              ...
              {atividades.map((item, i) => (
                <div key={i} className="activity-item">
                  <div>
                    ...
                    <br />
                    <span className="small text-muted-soft">{item.type} • {formatDate(item.date)}</span>
                  </div>
                  <span className={`fw-bold ${item.value < 0 ? 'text-danger' : 'text-success'}`}>
